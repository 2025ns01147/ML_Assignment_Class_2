# -*- coding: utf-8 -*-
"""ML_2025NS01147_Assignment2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/19jGxvHwFef8iSyHcKwfXbztEqlCYtk70
...
print('Click the button below and upload student_performance_dataset.csv')
...
"""## Step 5 - Feature (X) and Target (Y) - Split"""
...
if y_train.nunique() < 2 or y_test.nunique() < 2:
    print('\nWARNING: Either y_train or y_test (or both) contain only one class.')
    print('This prevents binary classification models from training correctly and is the likely cause of preceding errors.')
    print('Please ensure your dataset\'s \'Pass\' column has both 0s and 1s, and that it is correctly propagated to y_train and y_test.')
...
bars = ax.bar(['Fail (0)', 'Pass (1)'], counts_plot.values,
              color=['#f1c40f', '#9b59b6'], edgecolor='white', width=0.5)
...
for feat, val in corr.items():
    if pd.isna(val): # Check if value is NaN
        print(f"  {feat:20s} NaN (Cannot calculate correlation for 'Pass' due to zero variance in data.)")
        continue # Skip this iteration
    bar = '' * int(abs(val) * 20)
    sign = '+' if val >= 0 else '-'
    print(f"  {feat:20s} {sign}{abs(val):.3f}  {bar}")

"""## Step 10 - Boxplots - Pass vs Fail"""
...
"""## Step 11 - Pass Rate Analysis (For all 7 features)"""
...
def pass_rate_bar(ax, series_x, series_pass, title, x_labels=None):
    ...
    colors = plt.cm.Greens(pass_rates / 100)
...
"""## Step 15 -  MLP/ANN (Multi-Layer Perceptron)"""
...
"""## Step 18 - Gradient Boosting (Boosting ensemble)"""
...
"""## Step 19 - Voting Ensemble (RF + GB + SVM)"""
...
"""# Step 20 - Training & Learning Curves"""
...
plot_lc(axes[0], LogisticRegression(random_state=42,max_iter=500),
        X_train_sc, y_train, 'Logistic Regression', '#1abc9c')
...
if hasattr(mlp,'validation_scores_') and mlp.validation_scores_:
    axes[6].plot(range(1,len(mlp.validation_scores_)+1),
                 ...
                 's--', color='#ffbf00', lw=2, ms=5, label='Validation Loss')
...
axes[7].plot(gb.train_score_, 'o-', color='#81ecec', lw=2.5, ms=3, label='GB Train Score')
...
display(metrics_df.style.format('{:.2%}').background_gradient(cmap='Reds', axis=0))
...
"""## Step 22 - Performance Comparison"""
...
for i, metric in enumerate(['Accuracy', 'Precision', 'Recall', 'F1-Score']):
    sns.barplot(x=metrics_df.index, y=metrics_df[metric], ax=axes[i],
                palette='Reds', hue=metrics_df.index, legend=False)
...
for ax, (imp, title, color) in zip(axes, [
    (rf_imp, 'Random Forest', '#1abc9c'),
    (gb_imp, 'Gradient Boosting', '#e67e22'),
    (dt_imp, 'Decision Tree', '#ffbf00')
]):
...
"""### Step 25 - Specific Analysis: Sleep_Hours & Extra_Coaching Impact"""
...
axes[0].bar(sleep_analysis['Sleep_Hours'], sleep_analysis['Pass_Rate'],
            color=plt.cm.Purples(sleep_analysis['Pass_Rate'].values / 100),
...
bars = axes[1].bar(
    ...
    color=['#f1c40f', '#9b59b6'], edgecolor='white', width=0.4
)
...
"""# Step 26 - Save Model (for Streamlit App)"""
